...
// ulong = b4 b3 b2 b1
#define ULONG_BYTE4(u) ((u & 0xFF000000) >> 24) // highest byte
#define ULONG_BYTE3(u) ((u & 0xFF0000) >> 16)
#define ULONG_BYTE2(u) ((u & 0xFF00) >> 8)
#define ULONG_BYTE1(u) (u & 0xFF)		// lowest byte
...
struct ETHERNET_FRAME {
	unsigned char  dest[6];	// MAC of receiver
	unsigned char  src[6];	// MAC of sender
	unsigned short type;	// version: IPv4 0x0800, IPv6 0x86DD, ARP 0x0806
	unsigned char  data[];	// data
};

struct ETHERNET_ARP {
	unsigned short hrd;		// type (Ethernet), 0x0001.
	unsigned short pro;		// protocol (IP), 0x0800.
	unsigned char  hln;		// length of address (MAC), 6 bytes.
	unsigned char  pln;		// length of protocol IP, 4 bytes.
	unsigned short op;		// operation type {request, response} = {1, 2}.
	unsigned char  sha[6];	// sender address (MAC)
	unsigned char  spa[4];	// sender IP
	unsigned char  tha[6];	// receiver address (MAC)
	unsigned char  tpa[4];	// receiver IP
};

typedef struct _IPHeader {
	unsigned char  ver_len;		// ver and length of header
...
WSADATA			RS_WSAData;         // init WinSock
SOCKET			RS_SSocket;         // listener socket
char			RS_Hostname[128];   // host name
HOSTENT*		RS_Hostinfo;        // host info
SOCKADDR_IN		RS_SocketAddress;   // host addr
static u_long   	RS_Flag = 1;        // flg PROMISC
NetStat			RS_NetStat;
...
IPHeader * RS_Sniff() {
    IPHeader * hdr;
    int count = recv(RS_SSocket, (char*)&RS_Buffer[0], sizeof(RS_Buffer), 0);
